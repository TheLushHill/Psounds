...
def set_global(load_json, load_list, json_key_text, json_key_path, batch):
    ...
    if (load_json != "None"):
        ...
        g_load_format = "list"
        g_load_file = "Model\\Tools\\output_cache\\asr_opt\\demo.list"
...
# 先使用：
# set_global()  # 必须先设置全局变量
# b_load_file() # 接着加载数据文件
# 之后进行索引操作：
# b_change_index()   # 调整当前索引
# b_next_index()     # 或分页操作
# b_previous_index()
# 操作完索引操作数据
# b_submit_change()  # 文本修改
# b_merge_audio()    # 音频合并
# b_audio_split()    # 音频分割
# b_delete_audio()   # 删除操作
# 最后保存.list与.json
