...
import numpy as np
import gradio as gr
from Model.Tools.i18n.i18n import I18nAuto
import pandas as pd
i18n = I18nAuto(language=os.environ.get('language','Auto'))

def load_audio(file, sr):
    ffmpeg_path = os.path.abspath("Model/ffmpeg.exe")  # 根据实际安装位置调整
    if not os.path.exists(ffmpeg_path):
        raise FileNotFoundError(f"未找到 ffmpeg 可执行文件，请确保路径正确: {ffmpeg_path}")
    try:
        ...
        raise FileNotFoundError(f"音频文件未找到: {file}")
    except Exception as e:
        raise RuntimeError(f"音频加载失败: {file}, 错误: {e}")
