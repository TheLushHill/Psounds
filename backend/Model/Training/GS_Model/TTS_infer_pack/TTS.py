...
sys.path.insert(0, os.path.join(now_dir, "Model/GPT_SoVITS"))
import ffmpeg
import os
...
class TTS_Config:
    default_configs={
                "device": "cpu",
                "is_half": False,
                "t2s_weights_path": "Model/Training/pretrained_models/s1bert25hz-2kh-longer-epoch=68e-step=50232.ckpt",
                "vits_weights_path": "Model/Training/pretrained_models/s2G488k.pth",
                "cnhubert_base_path": "Model/Training/pretrained_models/chinese-hubert-base",
                "bert_base_path": "Model/Training/pretrained_models/chinese-roberta-wwm-ext-large",
            }
    configs:dict = None
    ...
    def run(self, inputs:dict):
        ...
        try:
            ...
            for item in data:
                ...
                # torch.cuda.synchronize()      # 等待前面所有 GPU 操作完成
                # start.record()
                # 结束测试代码
                ...
                # 测试代码
                # end.record()
                # torch.cuda.synchronize()      # 等待 infer_panel 的 GPU 操作完成
                # print("这是测试t2s_model GPU 时间: {:.3f} ms".format(start.elapsed_time(end)))
                # 结束测试代码
