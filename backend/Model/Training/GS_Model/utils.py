...
def load_checkpoint(checkpoint_path, model, optimizer=None, skip_optimizer=False):
    ...
    if checkpoint_path is None:
        ...
        return model, optimizer, 0.0, 0  # 返回初始学习率和迭代次数
    ...
    try:
        ...
        if optimizer is not None and not skip_optimizer:
            if "optimizer" in checkpoint_dict and checkpoint_dict["optimizer"] is not None:
                try:
                    optimizer.load_state_dict(checkpoint_dict["optimizer"])
                except ValueError as e:
                    print(f"Optimizer loading skipped due to mismatch: {str(e)}")
        ...
        for k in model_state_dict:
            if k in saved_state_dict:
                if saved_state_dict[k].shape == model_state_dict[k].shape:
                    ...
                    mismatched_keys.append(f"{k} (shape {saved_state_dict[k].shape} vs {model_state_dict[k].shape})")
            else:
        ...
        # 打印详细加载报告 
        print(f"\nSuccessfully loaded {len(new_state_dict)}/{len(model_state_dict)} parameters")
        if missing_keys:
            print(f"Missing keys ({len(missing_keys)}):\n{', '.join(missing_keys[:3])}{'...' if len(missing_keys)>3 else ''}")
        if mismatched_keys:
            print(f"Shape mismatches ({len(mismatched_keys)}):\n{', '.join(mismatched_keys[:3])}{'...' if len(mismatched_keys)>3 else ''}")
        ...
        # 异常处理增强 
        print(f"Critical error loading checkpoint: {str(e)}")
        traceback.print_exc()
        return model, optimizer, 0.0, 0  # 返回安全值防止死循环
...
def get_hparams(init=True, stage=1):
    ...
    # config_path = "Model\\Training\\GS_Model\\configs\\s2.json"
